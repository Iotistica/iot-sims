...
// Watch both open and tag so the form reinitialises whenever either changes —
// this handles the case where the user clicks Edit on a second tag while the
// drawer is already open (open stays true, tag prop changes).
watch([() => props.open, () => props.tag], ([open]) => {
  if (!open) return
  skipBehaviorReset = true
  const src = props.tag
  if (src) {
    Object.assign(form, {
      data_type: src.data_type ?? 'Double',
    ...
    Object.assign(form, {
      data_type: props.meta.data_types[0] ?? 'Double',
      writable: false, name: '', unit: '', behavior: 'constant', enabled: true,
    })
...
async function save() {
  ...
  loading.value = true
  const body = { ...form, enabled: form.enabled ? 1 : 0, writable: form.writable, behavior_params: JSON.stringify(params.value) }
  try {
    if (props.tag) {
      await api.tags.update(props.deviceId, props.tag.id, body)
      message.success('Tag updated')
    } else {
      await api.tags.create(props.deviceId, body)
  ...
  >
    <a-form layout="vertical" :colon="false">
        ...
        <a-col :span="14">
          <a-form-item label="Data Type" required>
            <a-select v-model:value="form.data_type">
              <a-select-option v-for="t in meta.data_types" :key="t" :value="t">{{ t }}</a-select-option>
            </a-select>
          </a-form-item>
        </a-col>
        <a-col :span="10">
          <a-form-item label="Writable" style="margin-top:5px">
      ...
      <a-form-item label="Unit">
        <a-input v-model:value="form.unit" placeholder="e.g. degC, percent, kW" />
      </a-form-item>
