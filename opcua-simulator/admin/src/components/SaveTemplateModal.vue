...
function save() {
  ...
  const existing = JSON.parse(localStorage.getItem('opcua-sim-user-templates') || '[]')
  existing.push({
    key: `user-${Date.now()}`,
  ...
  localStorage.setItem('opcua-sim-user-templates', JSON.stringify(existing))
  ...
  >
    <a-form layout="vertical" style="margin-top:8px">
      ...
      <div style="font-size:12px;color:#8c8c8c">
        {{ tags.length }} tag{{ tags.length !== 1 ? 's' : '' }} will be saved
      </div>
    </a-form>
  </a-modal>
</template>
