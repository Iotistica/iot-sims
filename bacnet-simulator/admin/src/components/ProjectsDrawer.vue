...
import type { Project, BACnetConnectionConfig, BACnetDiscoveryConnection } from '../types'
import { api } from '../api'
...
async function loadProject(p: Project) {
  try {
    ...
    emit('loaded', p.id, p.name, p.description, result.connection_config ?? null, result.discovery_connections ?? null)
