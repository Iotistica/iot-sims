...
def _build_fmu_provider(
    ...
    runtime_url, timeout_s, runtime_api_key = get_runtime_settings(settings)
...
def recover_unhealthy_simulation_models(database: Any, engine: Any) -> dict[str, Any]:
    ...
    registration is missing entirely, reports status "error", or is stuck on
    a persistent HTTP-level failure (diagnostics.last_http_status >= 400,
    even though status is still "running" -- e.g. a sustained HTTP 5xx from
    the FMU runtime; status alone doesn't flip for that case, see the
    comment at its check below) -- e.g. after the FMU runtime process
    restarted mid-run and the session was lost, or a previous initialize()
    aborted because a Point mapping had no live value yet (see
    FMUInputResolutionError in providers/fmu.py). Deliberately checks
    last_http_status, not diagnostics.error -- error is also set for a
    perfectly normal warmup-in-progress response (a 200 reporting
    runtime_state != "RUNNING"), and keying off it caused exactly the
    failure mode this function exists to prevent: a still-warming-up model
    (e.g. a slow EnergyPlus-backed one) got torn down and restarted on
    every single sweep cycle, forever, never allowed to finish warming up.
    Genuinely healthy (RUNNING, no HTTP error) providers -- warming up or
    not -- are left completely untouched, so this never creates a duplicate
    session for an already-working model, and never interrupts a legitimate
    long warmup either.
    ...
    runtime_url, timeout_s, runtime_api_key = get_runtime_settings(settings)
    try:
        FMURuntimeClient(runtime_url, timeout_s, runtime_api_key).health()
    except Exception as exc:
        log.warning(
    ...
    for config in persisted:
        ...
        runtime_id = provider_runtime_id(config)
        provider_info = current_providers.get(runtime_id, {})
        status = provider_info.get("status")
        # status alone isn't enough: a provider stuck on a persistent
        # runtime error (e.g. an HTTP 5xx from the FMU runtime) never
        # leaves ProviderStatus.RUNNING -- step()'s own tick-gating check
        # (`if self._status != RUNNING: return`) needs it to stay RUNNING
        # so a transient failure keeps retrying every tick on its own; only
        # a real session loss (404) currently flips status away from it.
        # diagnostics.last_http_status is the signal to use here, NOT
        # diagnostics.error -- confirmed live, the hard way: error also gets
        # set for a perfectly normal warmup-in-progress response (a 200
        # reporting runtime_state != "RUNNING", see fmu.py's step()), so
        # keying off error alone made this sweep tear down and restart any
        # still-warming-up model on every single cycle, forever, before it
        # could ever finish (confirmed: an EnergyPlus-backed model logging
        # "FMU model started" every ~30s, one sweep interval, never
        # progressing). last_http_status is only ever set from a real HTTP
        # response and only counts as unhealthy at >=400, which normal
        # warmup responses (200) never are.
        diagnostics = provider_info.get("diagnostics") or {}
        last_http_status = diagnostics.get("last_http_status")
        provider_has_error = last_http_status is not None and last_http_status >= 400
        if status == "running" and not provider_has_error:
        ...
        if runtime_id not in current_providers:
            reason = "missing"
        elif provider_has_error:
            reason = f"status={status} last_http_status={last_http_status} error={diagnostics.get('error')}"
        else:
            reason = str(status)
        log.info(
