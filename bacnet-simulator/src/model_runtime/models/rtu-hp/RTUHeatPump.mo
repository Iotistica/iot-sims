...
within ;
model RTUHeatPump
  "Packaged rooftop unit: DX cooling, air-source heat-pump heating, economizer, VAV-capable supply fan"
  ...
  // with its own self-contained DX cooling and heat-pump heating -- no
  ...
  // circuit and heat-pump heating section.
  // ===========================================================================
  ...
  // ---------------------------------------------------------------------------
  // Heat-pump heating parameters
  //
  // The air side remains an idealized sensible HeaterCooler_u for FMU
  // robustness. Heating capacity and compressor electrical power are derived
  // algebraically from outdoor-air temperature TOut.
  // ---------------------------------------------------------------------------
  parameter Modelica.Units.SI.HeatFlowRate QHea_flow_nominal = 30000
    "Nominal sensible heat-pump heating capacity";

  parameter Real COPHea_nominal(unit="1") = 3.2
    "Heating COP at TOutHeaCOPRef; calibration placeholder, not a manufacturer curve";

  parameter Modelica.Units.SI.Temperature TOutHeaCOPRef = 280.15
    "Outdoor-air reference temperature for heating COP (7 degC)";

  parameter Real kHeaCOPPerKelvin(unit="1/K") = 0.06
    "Linear heating-COP change per kelvin relative to TOutHeaCOPRef; positive means COP falls as outdoor temperature falls";

  parameter Real COPHea_min(unit="1") = 1.2
    "Lower bound on heat-pump heating COP";

  parameter Real COPHea_max(unit="1") = 4.5
    "Upper bound on heat-pump heating COP";

  parameter Modelica.Units.SI.Temperature TOutHeaCapRef = 280.15
    "Outdoor-air reference temperature for heating-capacity modifier (7 degC)";

  parameter Real kHeaCapPerKelvin(unit="1/K") = 0.015
    "Linear available-heating-capacity change per kelvin relative to TOutHeaCapRef";

  parameter Real heaCapModifierMin(min=0, max=1) = 0.50
    "Lower bound on available heat-pump heating capacity as a fraction of nominal";

  parameter Real heaCapModifierMax(min=0, max=1) = 1.00
    "Upper bound on available heat-pump heating capacity as a fraction of nominal";
  ...
  Modelica.Blocks.Interfaces.RealInput TOut(
    final unit="K",
    displayUnit="degC")
    "Outdoor-air temperature; used for economizer control, mixed-air calculation,
    cooling COP/capacity approximation, and heat-pump heating COP/capacity";
  ...
  Modelica.Blocks.Interfaces.RealOutput QHeaLoad(
    final unit="W")
    "Heat-pump heating delivered to the air";
  ...
  Modelica.Blocks.Interfaces.RealOutput heatingPLR(
    final unit="1")
    "Heating part-load ratio diagnostic after outdoor-temperature capacity derating.";

  Modelica.Blocks.Interfaces.RealOutput PHeatingCompressor(
    final unit="W")
    "Heat-pump compressor electrical power while heating";

  Modelica.Blocks.Interfaces.RealOutput totalElectricPower(
    final unit="W")
    "PCompressor + PHeatingCompressor + PFan";

  Modelica.Blocks.Interfaces.RealOutput heatingCOP(
    final unit="1")
    "Effective heat-pump heating COP as a bounded function of outdoor temperature; 0 when heating is off";

  Modelica.Blocks.Interfaces.RealOutput availableHeatingCapacity(
    final unit="W")
    "Positive available sensible heating capacity after outdoor-temperature derating";
  ...
  Modelica.Blocks.Interfaces.BooleanOutput heatingStatus
    "True when heat-pump heating is active (heatingPLR > 0.01). Maps to the
...
protected
  ...
  Real heatingCapacityModifier(unit="1")
    "Bounded heat-pump heating-capacity modifier from outdoor temperature";
  Real heatingCoilCommand(unit="1")
    "Actual normalized heating-coil command after outdoor-temperature derating";
  Real steadyStateHeatingCOP(unit="1")
    "Outdoor-temperature-adjusted heating COP before on/off gating";
  ...
  // ---------------------------------------------------------------------------
  // Heat-pump heating section -- idealized sensible heater. Outdoor-temperature
  // capacity and COP are represented algebraically to preserve FMU robustness,
  // analogous to the simplified DX-cooling treatment below.
  // ---------------------------------------------------------------------------
  Buildings.Fluid.HeatExchangers.HeaterCooler_u heaCoi(
    ...
    dp_nominal=50,
    energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial)
    "Idealized sensible air-source heat-pump heating section";
...
equation
  ...
  // ---------------------------------------------------------------------------
  // Heat-pump heating outputs
  //
  // TOut is supplied by the calling application. As outdoor temperature falls,
  // both available heating capacity and COP decrease according to bounded,
  // calibratable linear maps. No detailed refrigerant reversing-cycle states
  // are introduced.
  // ---------------------------------------------------------------------------
  heatingCapacityModifier =
    max(heaCapModifierMin, min(heaCapModifierMax,
      1 + kHeaCapPerKelvin*(TOut - TOutHeaCapRef)));

  heatingCoilCommand = min(1, yHea * heatingCapacityModifier);
  heaCoi.u = heatingCoilCommand;

  availableHeatingCapacity = QHea_flow_nominal * heatingCapacityModifier;
  QHeaLoad = heaCoi.Q_flow;
  heatingPLR = heatingCoilCommand;

  steadyStateHeatingCOP =
    max(COPHea_min, min(COPHea_max,
      COPHea_nominal + kHeaCOPPerKelvin*(TOut - TOutHeaCOPRef)));

  heatingCOP =
    if QHeaLoad > 1.0 then steadyStateHeatingCOP else 0;

  PHeatingCompressor =
    if QHeaLoad > 1.0 then QHeaLoad/heatingCOP else 0;

  totalElectricPower = PCompressor + PHeatingCompressor + PFan;
...
<p>
Experimental packaged rooftop unit (RTU): DX cooling, air-source heat-pump heat, dry-bulb
...
the same component this model's own heat-pump heating section uses, and the same
...
<h4>Heat-pump heating</h4>
<p>
Heating keeps the idealized <code>Buildings.Fluid.HeatExchangers.HeaterCooler_u</code>
air-side component for FMU robustness. The calling application supplies
<code>TOut</code>, and the RTU uses it internally to calculate a bounded
heating-capacity modifier and a bounded heating COP. <code>QHeaLoad</code>
is the sensible heat delivered to the air, and
<code>PHeatingCompressor = QHeaLoad/heatingCOP</code> represents compressor
electrical power in heating mode. The default linear COP and capacity maps
are calibration placeholders, not manufacturer performance curves. This v1
does not include auxiliary electric resistance heat, defrost operation, or
detailed refrigerant reversing-cycle states.
</p>
<h4>VAV/CAV hook</h4>
<p>
...
<li>
No detailed refrigerant reversing-cycle states, no auxiliary electric heat,
no defrost cycle, no return/exhaust fan, no energy recovery, no
...
<li>
The detailed refrigerant/condenser side of the heat-pump/DX cycle is not modeled -- there is no
...
heatingPLR, PHeatingCompressor, totalElectricPower, heatingCOP, availableHeatingCapacity, compressorCOP,
